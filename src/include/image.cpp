...
//     assert(format == format_t::FORMAT_LDR || format == format_t::FORMAT_HDR);
...
//             if (image->format == format_t::FORMAT_LDR) {
...
//     image = image_create(width, height, channels, format_t::FORMAT_LDR);
//     if (is_rle) {
...
//     assert(image->format == format_t::FORMAT_LDR);
...
//     image = image_create(width, height, 3, format_t::FORMAT_HDR);
//     for (i = 0; i < height; i++) {
//         int row = height - 1 - i;
...
//     assert(image->format == format_t::FORMAT_HDR && image->channels >= 3);
