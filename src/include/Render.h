...
public:
    void triangle(Eigen::Matrix<float, 4, 3> &clipc, IShader &shader, TGAImage &image, TGAImage *zbuffer);

    Render(Model *m, Shader &sh, TGAImage *im, TGAImage* zb) : model(m), shader(sh), image(im), zbuffer(zb) {}
    void Rendering()
    {
